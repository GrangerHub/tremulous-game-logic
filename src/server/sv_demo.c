...
Copyright (C) 2015-2020 GrangerHub
...
static char savedGameMode[MAX_CVAR_VALUE_STRING] = "";
...
void SV_DemoWriteAllPlayerState(void)
{
	...
	for (i = 0; i < sv_maxclients->integer; i++)
	{
		...
		MSG_WriteDeltaPlayerstate(
			svs.clients[i].netchan.alternateProtocol, &msg, &sv.demoPlayerStates[i],
			player);
...
void SV_DemoWriteAllEntityState(void)
{
	...
	for (i = 0; i < sv.num_entities; i++)
	{
		...
		MSG_WriteDeltaEntity(
			svs.clients[i].netchan.alternateProtocol, &msg, &sv.demoEntities[i].s,
			&entity->s, qfalse);
...
void SV_DemoReadConfigString( msg_t *msg )
{
	...
	if ( num < CS_PLAYERS + sv_democlients->integer || num >= CS_PLAYERS + sv_maxclients->integer ) { // we make sure to not overwrite real client configstrings (else when the demo starts, normal players will have no name, no model and no status!) - this cannot be done at recording time because we can't know how many sv_maxclients will be set at replaying time
		SV_SetConfigstring(num, configstring, qfalse);
	}
}
...
Read a demo client configstring from a message, load it into memory and
broadcast changes to gamecode and clients
This function also manages demo clientbegin at connections and teamchange
(which are normally totally handled by the gamecode, so we can't directly access
nor store these events in the demo, we must use clever ways to reproduce them at
the right time)
====================
*/
void SV_DemoReadClientConfigString(msg_t *msg) {
	...
	// This part manages when a client should begin or be dropped based on the
	// configstrings. This is a workaround because begin and disconnect events are
	// managed in the gamecode, so we here use a clever way to know when these
	// events happen (this is based on a careful reading of how work the
	// mechanisms that manage players in a real game, so this should be OK in any
	// case).
	// Note: this part could also be used in userinfo instead of client
	// configstrings (but since DropClient automatically sets userinfo to null,
	// which is not the case the other way around, this way was preferred)
	if(
		Q_stricmp(sv.configstrings[CS_PLAYERS + num].s, configstring) &&
		configstring && strlen(configstring)) {
		int svdoldteam;
		int svdnewteam;

		// client begin or just changed team: previous configstring and new one are
		// different, and the new one is not null

		client = &svs.clients[num];

		// affect the new team if detected, else if an empty string is returned,
		// just set -1 (will allow us to detect that there's really no team change
		// instead of having 0 which is TEAM_NONE)
		svdoldteam =
			strlen(
				Info_ValueForKey(sv.configstrings[CS_PLAYERS + num].s, "t")) ?
				atoi(Info_ValueForKey(sv.configstrings[CS_PLAYERS + num].s, "t")) : -1;
		svdnewteam =
		strlen(Info_ValueForKey(configstring, "t")) ?
		atoi(Info_ValueForKey(configstring, "t")) : -1;

		// Set the client configstring (using a standard Q3 function)
		SV_SetConfigstring(CS_PLAYERS + num, configstring, qfalse);

		// Set some infos about this user:

		// to check if a client is a democlient, you can either rely on this
		// variable, either you can check if num (index of client) is >= CS_PLAYERS
		// + sv_democlients->integer && < CS_PLAYERS + sv_maxclients->integer (if
		// it's not a configstring, remove CS_PLAYERS from your if)
		svs.clients[num].demoClient = qtrue;
		// set the name (useful for internal functions such as status_f). Anyway
		// userinfo will automatically set both name (server-side) and netname
		// (gamecode-side).
		Q_strncpyz(
			svs.clients[num].name, Info_ValueForKey( configstring, "n" ),
			 MAX_COLORFUL_NAME_LENGTH);


		// DEMOCLIENT INITIAL TEAM MANAGEMENT
		// Note: needed only to set the initial team of the democlients, subsequent
		// team changes are directly handled by their clientCommands
		// DEPRECATED: moved to userinfo, more interoperable (because here team is
		// an int, while in userinfo the full team name string is stored and can be
		// directly replayed)

		if(!strlen(Info_ValueForKey(svs.clients[num].userinfo, "t"))) {
			if(
				configstring && strlen(configstring) &&
				...
				// If the client changed team, we manually issue a team change
				// (workaround by using a clientCommand team)
				char *svdnewteamstr = malloc( 30 * sizeof *svdnewteamstr );

				if (svdnewteam == 0) {
					strcpy(svdnewteamstr, "spectator");
				} else {
					dll_G_TeamStringFromTeamNumber(
						svdnewteam, svdnewteamstr, 30 * sizeof *svdnewteamstr);
				}

				// workaround to force the server's gamecode and clients to update the
				// team for this client - note: in fact, setting any team (except
				// spectator) will make the engine set the client to a random team, but
				// it's only sessionTeam! so the democlients will still be shown in the
				// right team on the scoreboard, but the engine will consider them in a
				// random team (this has no concrete adverse effect to the demo to my
				// knowledge)
				SV_ExecuteClientCommand(
					&svs.clients[num], va("team %s", svdnewteamstr), qtrue);

				free( svdnewteamstr );
			}
		}

		// Set the remoteAddress of this client to localhost (this will set
		// "ip\localhost" in the userinfo, which will in turn force the gamecode to
		// set this client as a localClient, which avoids inactivity timers and some
		// other stuffs to be triggered)
		...
		// Make sure the gamecode consider the democlients (this will allow to show
		// them on the scoreboard and make them spectatable with a standard follow)
		// - does not use argv (directly fetch client infos from userinfo) so no
		// need to tokenize with Cmd_TokenizeString()
		// Note: this also triggers the gamecode refreshing of the client's userinfo
		dll_ClientBegin(num);
	} else if(
		Q_stricmp(sv.configstrings[CS_PLAYERS + num].s, configstring) &&
		strlen(sv.configstrings[CS_PLAYERS + num].s) &&
		(!configstring || !strlen(configstring)) ) { 
		// client disconnect: different configstrings and the new one is empty, so
		// the client is not there anymore, we drop him (also we check that the old
		// config was not empty, else we would be disconnecting a player who is
		// already dropped)
		client = &svs.clients[num];
		SV_DropClient( client, "disconnected" ); // same as SV_Disconnect_f(client);
		SV_SetConfigstring(CS_PLAYERS + num, configstring, qfalse); // empty the configstring
	} else {
		// In any other case (should there be?), we simply set the client
		// configstring (which should not produce any error)
		SV_SetConfigstring(CS_PLAYERS + num, configstring, qfalse);
	}
}
...
Read a demo client userinfo string from a message, load it into memory, fills
client_t fields by parsing the userinfo and broacast the change to the gamecode
and clients
Note: this function also manage the initial team of democlients when demo
recording has started. Subsequent team changes will be directly handled by
clientCommands "team"
====================
*/
void SV_DemoReadClientUserinfo( msg_t *msg )
{
	...
	Q_strncpyz(
		svdoldteam, Info_ValueForKey(client->userinfo, "team"), MAX_NAME_LENGTH);
	...
	// other stuffs to be triggered)
	if(strlen(userinfo)) {
		// we check that the client isn't being dropped (in which case we shouldn't
		// set an address)
		NET_StringToAdr( "localhost", &client->netchan.remoteAddress, NA_LOOPBACK );
	}
	...
	// we need to prepend the userinfo command (or in fact any word) to tokenize
	// the userinfo string to the second index because SV_UpdateUserinfo_f expects
	// to fetch it with Argv(1)
	Cmd_TokenizeString( va("userinfo %s", userinfo) );
	// will update the server userinfo, automatically fill client_t fields and
	// then transmit to the gamecode and call ClientUserinfoChanged() which will
	// also update the gamecode's client_t from the new userinfo (eg: name
	// [server-side] and netname [gamecode-side] will be both updated)
	SV_UpdateUserinfo_f(client);


	// DEMOCLIENT INITIAL TEAM MANAGEMENT
	// Note: it is more interoperable to do team management here than in
	// configstrings because here we have the team name as a string, so we can
	// directly issue it in a "team" clientCommand
	// Note2: this function is only necessary to set the initial team for
	// democlients (the team they were at first when the demo started), for all
	// the latter team changes, the clientCommands are recorded and will be
	// replayed
	if(
		userinfo && strlen(userinfo) && strlen(svdnewteam) &&
	  (
			!strlen(svdoldteam) ||
			(Q_stricmp(svdoldteam, svdnewteam) && strlen(svdnewteam)))) {
		// if there was no team for this player before OR if the new team is
		// different
		// If the democlient changed team, we manually issue a team change
		// (workaround by using a clientCommand team)
		// workaround to force the server's gamecode and clients to update the team
		// for this client
		SV_ExecuteClientCommand(client, va("team %s", svdnewteam), qtrue);

	} else if (!strlen(svdoldteam) && !strlen(svdnewteam) && strlen(userinfo) &&
		   !strlen(Info_ValueForKey(sv.configstrings[CS_PLAYERS + num].s, "t"))
		   ) {
		// old and new team are not specified in the previous and current userinfo,
		// but a userinfo is present
		// Else if the democlient has no team specified, it's probably because he
		// just has connected and so he is set to the default team by the gamecode
		// depending on the gamecode: for >= GT_TEAM it's spectator, for all the
		// others non-team based game_modes it's direcly in-game
		// FIXME? If you are trying to port this patch and weirdly some democlients
		// are visible in scoreboard but can't be followed, try to uncomment these
		// lines
		// if it's a team-based game_mode, by default players are spectators
		// (unless g_teamAutoJoin was set)
		if(demoTeamAutoJoin <= 0) {
			// send to spectator the client (prevent the democlient from being
			// followed)
			SV_ExecuteClientCommand(client, "team spectator", qtrue);
		} else { // else by default they join the game
			SV_ExecuteClientCommand(client, "team auto", qtrue);
		}
	}
...
void SV_DemoReadAllEntityState(msg_t *msg) {
	...
	while(1) {
		...
		MSG_ReadDeltaEntity(
			svs.clients[num].netchan.alternateProtocol, msg, &sv.demoEntities[num].s,
			&entity->s, num);
...
Read all shared entities (gentity_t or sharedEntity_t->entityShared_t - NOTE:
sharedEntity_t = gentity_t != entityShared_t which is a subfield of gentity_t)
from a message and store them in a demoEntities[num].r array (it will be loaded
in memory later when SV_DemoReadRefresh() is called)
====================
*/
void SV_DemoReadAllEntityShared(msg_t *msg) {
	...
	while(1) {
		...
		// fix bots camera freezing issues - because since now the engine will
		// consider these democlients just as normal players, it won't be using
		// anymore special bots fields and instead just use the standard viewangles
		// field to replay the camera movements
...
read_next_demo_event: // used to read next demo event
		...
		// Get a message
		r = FS_Read(&msg.data, 4, sv.demoFile);
		if (r != 4)
		{
...
void SV_DemoAutoDemoRecord(void)
{
	...
	Com_Printf(
		"DEMO: automatic recording server-side demo to: %s/svdemos/%s.%s%d\n",
		strlen(Cvar_VariableString("fs_game")) ? 
			Cvar_VariableString("fs_game") :
			BASEGAME,
		demoname, SVDEMOEXT, PROTOCOL_VERSION);
	SV_SendServerCommand(
		NULL, "chat -1 0 \"^3DEMO: automatic recording server-side demo to: %s.%s%d.\"",
		demoname, SVDEMOEXT, PROTOCOL_VERSION );
...
====================
SV_DemoStartRecord
Start the recording of a demo by saving some headers data (such as the number of clients, mapname, game_mode, etc..)
...
void SV_DemoStartRecord(void)
{
	...
	// Write g_game_mode
	MSG_WriteString(&msg, "g_game_mode");
	Cvar_VariableStringBuffer("g_game_mode", buffer, sizeof(buffer));
	MSG_WriteString(&msg, buffer);
	...
	for (i = 0; i < MAX_CONFIGSTRINGS; i++)
	{
		if ( &sv.configstrings[i] ) { // if the configstring pointer exists in memory (because we will check all the possible indexes, but we don't know if they really exist in memory and are used or not, so here we check for that)
			SV_DemoWriteConfigString(i, sv.configstrings[i].s);
		}
	}
	...
	SV_SendServerCommand(
		NULL, "chat -1 0 \"^3DEMO: Recording server-side demo %s.\"", sv.demoName);
}
...
void SV_DemoStopRecord(void)
{
	...
	SV_SendServerCommand(
		NULL, "chat -1 0 \"^3DEMO: Stopped recording server-side demo %s.\"",
		sv.demoName);
}
...
Just issue again the demo_play command along with the demo filename (used when the system will automatically restart the server for special cvars values to be set up, eg: sv_maxclients, fs_game, g_game_mode, etc..)
====================
*/
...
This function will also check that everything is alright (such as game_mode, map, sv_fps, sv_democlients, sv_maxclients, etc.), if not it will try to fix it automatically
...
{
	msg_t msg;
	int r, time, i, clients, fps, timelimit, fraglimit, capturelimit;

	char *game_mode = malloc( MAX_QPATH * sizeof *game_mode );
	...
	r = i = clients = fps = timelimit = fraglimit = capturelimit = 0;
	time = 400;
	...
	while( Q_stricmp(metadata, "endMeta")  )
	{
		...
		} else if ( !Q_stricmp(metadata, "g_game_mode") ) {
			// reading g_game_mode (from the demo)
			Q_strncpyz(game_mode, MSG_ReadString(&msg), MAX_QPATH);
			// memorize the current game_mode
			if(!keepSaved) {
				char buffer[MAX_CVAR_VALUE_STRING];
				// save the game_mode before switching
				Cvar_VariableStringBuffer("g_game_mode", buffer, sizeof(buffer));
				Q_strncpyz(savedGameMode, buffer, MAX_CVAR_VALUE_STRING);
			}
		...
		} else if ( !Q_stricmp(metadata, "mapname") ) {
	...
	// Remove g_doWarmup (bugfix: else it will produce a weird bug with all game_modes except CTF and Double Domination because of CheckTournament() in g_main.c which will make the demo stop after the warmup time)
	...
	if ( !sv_demoTolerant->integer ) // print the meta datas only if we're not in faults tolerance mode (because if there are missing meta datas, the printing will throw an exception! So we'd better avoid it)
		Com_Printf("DEMO: Details of %s recorded %s on server \"%s\": sv_fps: %i initial_servertime: %i clients: %i fs_game: %s g_game_mode: %s map: %s timelimit: %i fraglimit: %i capturelimit: %i \n", sv.demoName, datetime, hostname, fps, time, clients, fs, game_mode, map, timelimit, fraglimit, capturelimit);

	// Checking if all initial conditions from the demo are met (map, sv_fps, game_mode, servertime, etc...)
	...
	if ( !com_sv_running->integer || Q_stricmp(sv_mapname->string, map) ||
	    ...
	    (Q_stricmp(Cvar_VariableString("g_game_mode"), game_mode)) // check for game_mode change (need a restart to take effect since it's a latched var) AND check that the game_mode difference is not between SinglePlayer and DM/FFA, which are in fact the same game_mode (and the server will automatically change SinglePlayer to FFA, so we need to detect that and ignore this automatic change)
	   ) {
		...
		if((Q_stricmp(Cvar_VariableString("g_game_mode"), game_mode) && strlen(game_mode))) {
			Q_strncpyz(
				savedGameMode, (const char*)Cvar_VariableString("g_game_mode"), MAX_CVAR_VALUE_STRING);
		}

		Cbuf_AddText(va("devmap %s\n", map)); // Change map (using devmap to enable cheats)
	...
	free(datetime);
	free(game_mode);
	//free( metadata ); // it seems glibc already frees this pointer automatically
	// since the malloc was removed, if we specify this line we'll get a crash

	// Start reading the first frame
	// log that the demo is started here
	Com_Printf("Playing demo %s.\n", sv.demoName);
	// send a message to player
	SV_SendServerCommand(NULL, "chat -1 0 \"^3DEMO: Server-side demo replay started!\"");
	// send a centerprint message to player
	SV_SendServerCommand(NULL, "cp \"^3DEMO: Server-side demo replay started!\" 0");
	sv.demoState = DS_PLAYBACK; // set state to playback
	Cvar_SetValue("sv_demoState", DS_PLAYBACK);
	// Don't save values anymore: the next time we stop playback, we will restore
	// previous values (because now we are really launching the playback, so
	// anything that might happen now is either a big bug or the end of demo, in
	// any case we want to restore the values)
	keepSaved = qfalse;
	// reading the first frame, which should contain some initialization events
	// (eg: initial confistrings/userinfo when demo recording started, initial
	// entities states and placement, etc..)
	SV_DemoReadFrame();
}
...
void SV_DemoStopPlayback(void)
{
	...
	if (olddemostate == DS_PLAYBACK) { // unload democlients only if we were replaying a demo (if not it will produce an error!)
		for (i = 0; i < sv_democlients->integer; i++)
			SV_SetConfigstring(CS_PLAYERS + i, NULL, qfalse); //qtrue
	}
	...
	if ( !keepSaved ) {
		...
		if(Q_stricmp(savedGameMode, "")) {
			Cvar_SetLatched("g_game_mode", va("%s", savedGameMode) );
			Q_strncpyz(savedGameMode, "", MAX_CVAR_VALUE_STRING);
		}
	}
