...
void CM_TracePointThroughPatchCollide( traceWork_t *tw, const struct patchCollide_s *pc ) {
	...
	planes2 = pc->planes;
	for ( i = 0 ; i < pc->numPlanes ; i++, planes2++ ) {
		offset = DotProduct( tw->offsets[ planes2->signbits ], planes2->plane );
		d1 = DotProduct( tw->start, planes2->plane ) - planes2->plane[3] + offset;
		d2 = DotProduct( tw->end, planes2->plane ) - planes2->plane[3] + offset;
		if ( d1 <= 0 ) {
	...
	for ( i = 0 ; i < pc->numFacets ; i++, facet++ ) {
		...
		if ( j == facet->numBorders ) {
...
#endif //BSPC
			planes2 = &pc->planes[facet->surfacePlane];
			...
			VectorCopy( planes2->plane,  tw->trace.plane.normal );
			tw->trace.plane.dist = planes2->plane[3];
		}
	}
}
...
void CM_TraceThroughPatchCollide( traceWork_t *tw, const struct patchCollide_s *pc ) {
	...
	for ( i = 0 ; i < pc->numFacets ; i++, facet++ ) {
		...
		//
		planes2 = &pc->planes[ facet->surfacePlane ];
		VectorCopy(planes2->plane, plane);
		plane[3] = planes2->plane[3];
		if ( tw->type == TT_CAPSULE ) {
			...
			offset = DotProduct( tw->offsets[ planes2->signbits ], plane);
			plane[3] -= offset;
		...
		for ( j = 0; j < facet->numBorders; j++ ) {
			planes2 = &pc->planes[ facet->borderPlanes[j] ];
			if (facet->borderInward[j]) {
				VectorNegate(planes2->plane, plane);
				plane[3] = -planes2->plane[3];
			}
			else {
				VectorCopy(planes2->plane, plane);
