...
static qboolean PM_CheckPounce( void )
{
  ...
  if(pm->ps->pm_flags & PMF_LAUNCHING) {
    int delta = ((pm->ps->misc[ MISC_MISC3 ] + pm->ps->misc[ MISC_MISC ]) * pml.msec) / 75;
...
static qboolean PM_CheckJump( vec3_t customNormal )
{
  ...
  if(pm->ps->stats[ STAT_STAMINA ] > 0 //derp doesn't have STAMINA_MIN_TO_JUMP :/
		...
		if (cpm_pm_jump_z)
    {
      ...
      {
				jumpMagnitude += (cpm_pm_jump_z * BG_Class( pm->ps->stats[ STAT_CLASS ] )->jumpMagnitude);
...
static void PM_Pain_Saw_Pull( void ) {
  ...
  if(trace.fraction < 1.0f || trace.startsolid) {
    ...
    if(
      ...
      (BG_Weapon(pm->ps->weapon)->overheatType == OVERHEAT_FROM_CONTACT) &&
      BG_Weapon(pm->ps->weapon)->overheatAltMode) {
      pm->ps->misc[MISC_MISC3] += BG_Weapon(pm->ps->weapon)->repeatRate2;
...
static void PM_Weapon( void )
{
  ...
  if(BG_Weapon(pm->ps->weapon)->weaponOptionA == WEAPONOPTA_SPINUP) {
    if(
      ...
      (pm->ps->stats[STAT_WEAPON] == WP_HBUILD)) {
  ...
  if( BG_Weapon( pm->ps->weapon )->hasAltMode &&
      ( pm->ps->weapon == WP_ALEVEL1_UPG ) &&
      ( pm->ps->stats[ STAT_WEAPONTIME2 ] <= 0 ) && attack2 )
  {
