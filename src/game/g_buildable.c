    ...
    for( i = MAX_CLIENTS, ent = g_entities + i; i < level.num_entities; i++, ent++ ) {
      ...
      if( (BG_Buildable( ent->s.modelindex )->role & ROLE_POWER_SOURCE) &&
        ...
        if( !( BG_Buildable( ent->s.modelindex )->role &  ROLE_PERVASIVE ) &&
            !G_Visible( self, ent, *Temp_Clip_Mask(MASK_DEADSOLID, 0) ) ) {
          continue;
        }
...
qboolean AHovel_Blocked( gentity_t *hovel, gentity_t *player, qboolean provideExit )
{
  ...
  SV_Trace(
    &tr, position, mins, maxs, target, hovel->s.number, qfalse,
    *Temp_Clip_Mask(MASK_PLAYERSOLID, 0), TT_AABB );
...
void G_PositionHovelsBuilder( gentity_t *self )
{
  ...
  SV_Trace( &tr, self->r.currentOrigin, mins, maxs, hovelOrigin, self->s.number,
            qfalse, *Temp_Clip_Mask(MASK_PLAYERSOLID, 0), TT_CAPSULE );
...
qboolean ASlimer_CheckTarget( gentity_t *slime, gentity_t *target )
{
  ...
  if( !G_Visible( slime, target, *Temp_Clip_Mask(CONTENTS_SOLID, 0) ) )
    return qfalse;
...
void HMedistat_Think( gentity_t *self )
{
  ...
  if( self->spawned )
  {
    ...
    for( i = 0; i < num; i++ )
    {
      ...
      if( !( player->r.contents & MASK_SHOT ) ||
          ( player->r.contents & CONTENTS_ASTRAL ) )
        continue;
...
static qboolean HMGTurret_TrackEnemy( gentity_t *self )
{
  ...
  //if not pointing at our target then move accordingly
  if( angularDiff[ YAW ] < 0 && angularDiff[ YAW ] < ( -angularYawSpeed ) )
    self->s.angles2[ YAW ] += angularYawSpeed;
  else if( angularDiff[ YAW ] > 0 && angularDiff[ YAW ] > angularYawSpeed )
    self->s.angles2[ YAW ] -= angularYawSpeed;
  else
  ...
  for( i = MAX_CLIENTS, ent = g_entities + i; i < level.num_entities; i++, ent++ )
  {
    ...
    if(
      ...
      (
        ...
        ent->s.modelindex == BA_H_SPAWN ||
        ent->s.modelindex == BA_H_TELEPORTER ) )
    {
