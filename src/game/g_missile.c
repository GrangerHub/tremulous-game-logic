...
void G_ExplodeMissile(gentity_t *ent) {
  ...
  if(BG_Missile(weapon, mode)->detonate_saved_missiles) {
    ...
    for(i = 0; i < num; i++) {
      ...
      if((targ->s.weapon != weapon) || (targ->s.generic1 != mode)) {
        continue;
      }
      ...
      if(
        ...
        (ent->splashRadius / 2)) {
        targ->think = G_ExplodeMissile;
        targ->nextthink =
          level.time +
          BG_Missile(weapon, mode)->detonate_saved_missiles_delay;
      } else if(
        targ->nextthink > level.time +
        (2 * BG_Missile(weapon, mode)->detonate_saved_missiles_delay)) {
        targ->think = G_ExplodeMissile;
        targ->nextthink =
          level.time +
          (2 * BG_Missile(weapon, mode)->detonate_saved_missiles_delay);
    ...
    other->client->ps.stats[STAT_TEAM] != ent->buildableTeam) {
      switch(BG_Missile(weapon, mode)->impede_move) {
        ...
        case IMPEDE_MOVE_LOCK:
          ...
          AngleVectors(other->client->ps.viewangles, dir, NULL, NULL);
          other->client->ps.stats[STAT_VIEWLOCK] = DirToByte(dir);
          G_Entity_UEID_set(&other->client->last_trapper_id, attacker);
          break;
